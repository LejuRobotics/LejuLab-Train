...
parser.add_argument(
    "--input_quat_xyzw",
    action="store_true",
    help="Set this flag if input CSV quaternion columns are in xyzw order. They will be converted to wxyz.",
)
...
class MotionLoader:
    def __init__(
        ...
        self.input_fps = input_fps  # CSV files don't contain fps, must be provided via parameter
    ...
    def _load_motion(self):
        ...
        if args_cli.input_quat_xyzw:
            root_rot = root_rot[:, [3, 0, 1, 2]]
            print("Converted input root quaternion order from xyzw to wxyz.")
        ...
        csv_num_joints = dof_pos.shape[1]
        if self.num_robot_joints is not None:
            if csv_num_joints != self.num_robot_joints:
                raise ValueError(
                    f"CSV file has {csv_num_joints} joints, but robot model has {self.num_robot_joints} joints. "
                    f"Please ensure CSV file has the same number of joints as the robot."
                )
        print(
            f"Using all PKL joints (sequential mapping): {csv_num_joints} joints "
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    robot = scene["robot"]
    npz_to_isaac_indices = robot.find_joints(
        scene.cfg.robot.preserve_joint_order.joint_names, 
        preserve_order=True)[0]
    num_robot_joints = len(scene.cfg.robot.preserve_joint_order.joint_names)
