...
from isaaclab.utils.noise import AdditiveUniformNoiseCfg as Unoise
from leju_robot.tasks.locomotion.velocity.config.robanS17.robanS17 import RobanS17_CFG

from leju_robot.tasks.locomotion.velocity.terrains.rough import ROBAN_ROUGH_TERRAINS_CFG
...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        ...
        terrain_generator=ROBAN_ROUGH_TERRAINS_CFG,
    ...
    # robots
    robot: ArticulationCfg = RobanS17_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
    # sensors
...
class ObservationsCfg:
    ...
    class PrivilegedCfg(ObsGroup):
        ...
        # 保留 height_scanner sensor 与实现，仅注释此 obs term（移除约187维地形扫描）。
        # height_scan = ObsTerm(
...
class RewardsCfg:
    ...
    dof_power_l2 = RewTerm(func=mdp.joint_power_l2, weight=-5.0e-6)
    dof_torques_l2 = RewTerm(
        func=mdp.joint_torques_l2,
        weight=-3.0e-6,
        params={
            "asset_cfg": SceneEntityCfg(
                "robot", joint_names=["leg_[l,r][1-6]_joint", "zarm_.*_joint"]
            )
        },
    )
    ...
    feet_descent_velocity = RewTerm(
        func=mdp.feet_descent_velocity,
        weight=-0.5,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names="leg_[l,r]6_link"),
            "asset_cfg": SceneEntityCfg("robot", body_names="leg_[l,r]6_link"),
    ...
    joint_deviation_hip = RewTerm(
        ...
        weight=-1.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["leg_[l,r][2,3]_joint"])
        },
    )
    joint_deviation_hip_yaw = RewTerm(
        ...
        weight=-1.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["leg_[l,r]3_joint"])
        },
    )
    ...
    joint_deviation_arm_shouler_roll = RewTerm(
        func=mdp.joint_deviation_l1,
        weight=-1,
        params={
            "asset_cfg": SceneEntityCfg(
...
class TerminationsCfg:
    ...
    bad_orientation = DoneTerm(
        ...
        params={
            "limit_angle": 0.8,  # D16'': π/2*0.8 (≈72°) → 0.8 rad (≈46°)，对齐
            "asset_cfg": SceneEntityCfg("robot"),
        },
    )
...
class EventCfg:
    ...
    add_joint_default_pos = EventTerm(
        ...
        mode="startup",
        params={
            "asset_cfg": RobanS17_CFG.preserve_joint_order,
    ...
    base_external_force_torque = EventTerm(
        ...
        params={
            ...
            "probability": 0.002,  # ≈ 每 500 step (~10s) 触发一次脉冲
        },
    )
...
@configclass
class RobanS17RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.actions.joint_pos.joint_names = RobanS17_CFG.preserve_joint_order.joint_names
        self.observations.policy.joint_pos_rel.params = {"asset_cfg": RobanS17_CFG.preserve_joint_order}
        self.observations.policy.joint_vel_rel.params = {"asset_cfg": RobanS17_CFG.preserve_joint_order}
        self.observations.critic.joint_pos_rel.params = {"asset_cfg": RobanS17_CFG.preserve_joint_order}
        self.observations.critic.joint_vel_rel.params = {"asset_cfg": RobanS17_CFG.preserve_joint_order}

@configclass
class RobanS17RoughEnvCfg_PLAY(RobanS17RoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
