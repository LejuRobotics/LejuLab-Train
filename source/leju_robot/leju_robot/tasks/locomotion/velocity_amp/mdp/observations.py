...
from isaaclab.utils.math import quat_rotate_inverse, yaw_quat,quat_apply_yaw
...
def feet_contact_force(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg,
    ...
    # return contact_force_w.flatten(1, 2)
    asset: Articulation = env.scene[asset_cfg.name]
    root_link_quat_w = asset.data.root_link_quat_w.unsqueeze(1).repeat(1, contact_force_w.shape[1], 1)
    contact_force_b = quat_rotate_inverse(root_link_quat_w, contact_force_w)
...
def feet_lin_vel(
    ...
    root_link_quat_w = asset.data.root_link_quat_w.unsqueeze(1).repeat(1, body_lin_vel_w.shape[1], 1)
    body_lin_vel_b = quat_rotate_inverse(root_link_quat_w, body_lin_vel_w)
