...
class Runner:
    ...
    def _generate_models(
        ...
        for agent_id in possible_agents:
            if not state_spaces[agent_id]:
                state_spaces[agent_id] = observation_spaces[agent_id]
            ...
            if separate:
                for role in models_cfg:
                    ...
                    if agent_class == "amp" and role == "discriminator":
                        try:
                            state_space = env.amp_observation_space
                        except Exception as e:
                            logger.warning(
                                "Unable to get AMP space via 'env.amp_observation_space'. Using 'env.state_space' instead"
                            )
                    # TODO 是否需要额外操作，例如判断state_space为空，替换为observation space?
                    # print model source
                    source = model_class(
    ...
    def _generate_agent(
        ...
        for agent_id in possible_agents:
            if cfg["agent"]["use_symmetry_loss"]:
                memories[agent_id] = memory_class(num_envs=num_envs, device=device, memory_size=cfg["memory"]["memory_size"]*2)
            else:
        ...
        if agent_class in ["amp"]:
            ...
            agent_kwargs = {
                ...
                "discriminator_obs_history_length":env._env.env.cfg.num_amp_observations,
            }
