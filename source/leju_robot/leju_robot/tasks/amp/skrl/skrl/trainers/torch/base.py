...
class Trainer(ABC):
    ...
    def non_simultaneous_train(self) -> None:
        ...
        states = infos["observations"]["critic"] if self.env.state_space else observations
        ...
        for timestep in tqdm.tqdm(
            ...
            with torch.no_grad():
                # compute actions
                actions = self.agents.act(observations, timestep=timestep, timesteps=self.timesteps)[0]

                # step the environments
                next_observations, rewards, terminated, truncated, infos = self.env.step(actions)
                next_states = infos["observations"]["critic"] if self.env.state_space else next_observations
    ...
    def non_simultaneous_eval(self) -> None:
        ...
        for timestep in tqdm.tqdm(
            ...
            with torch.no_grad():
                # compute actions
                outputs = self.agents.act(observations, timestep=timestep, timesteps=self.timesteps)
